    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if(float(dst)>20.00 or float(dst)<50.00):
                ...
                pass
            # if(float(dst)<20.00):
            # 	Ttext="Take two steps backward"
            # 	language = 'en'
            # 	myobj = gTTS(text=Ttext, lang=language, slow=False) 
            # 	myobj.save("from_file3.mp3") 
            # 	os.system("from_file3.mp3")
            # elif(float(dst)>50.00):
            # 	Ttext="Please come a bit closure to the Camera"
            # 	language = 'en'
            # 	myobj = gTTS(text=Ttext, lang=language, slow=False) 
            # 	myobj.save("from_file4.mp3") 
            # 	os.system("from_file4.mp3")
            # else:
